...
interface MatchCardProps extends ComponentPropsWithoutRef<"button"> {
  match: MatchPreview;
}
  ...
  ({ match, className, ...props }, ref) => {
    ...
    return (
      <button
        ref={ref}
        type="button"
        className={`relative isolate mb-1.5 block h-[108px] w-full cursor-pointer overflow-hidden text-left outline-none ${MATCH_PAGE_CLASSES.cardBackground} backdrop-blur-[1px] transition-transform focus-visible:ring-2 focus-visible:ring-[#42EEC7]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 sm:h-28 ${className ?? ""}`}
        {...props}
      >
        <div
          ...
          <div className="absolute right-1 top-1/2 z-40 w-[34%] -translate-y-1/2 text-right sm:right-2 sm:w-[32%] lg:left-1/2 lg:right-auto lg:w-[30%] lg:-translate-x-1/2 lg:text-center">
            ...
            <p className="mt-0.5 text-[20px] font-semibold leading-none tracking-[0.03em] text-white sm:mt-1 sm:text-[24px] lg:mt-2 lg:text-[32px]">
              <span
                className={
                  resultTone === "Win"
                    ? MATCH_RESULT_CLASSES.Win.accentText
                    : ""
                }
              >
                {match.teamScore}
